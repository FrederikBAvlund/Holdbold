...
export async function POST(request: Request, { params }: { params: { userId: string } }) {
  ...
  if (!membership || membership.role !== "ADMIN") {
    return NextResponse.json({ error: "Kun admin kan godkende betaling" }, { status: 403 });
  }
  ...
  await prisma.notification.create({
    data: {
      ...
      teamId: body.teamId,
      type: "FINE",
      title: "Betaling godkendt",
      body: `Din betaling på ${total} kr er godkendt.`,
      link: "/dashboard/boder"
    }
  });

  return NextResponse.json({ approved: pendingFines.length, total });
}
